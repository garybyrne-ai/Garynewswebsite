        ...
        <article class="sigcard <?= !empty($s['signal_warmup']) ? 'sigcard--warmup' : '' ?>" style="--h:<?= $hue ?>;--c:<?= e($top['colour'] ?? '#139a5c') ?>" data-story="<?= e($s['id']) ?>">
          ...
          <div class="sigcard__body">
            ...
            <div class="sigcard__quick" data-vote data-story="<?= e($s['id']) ?>" data-total="<?= (int)$s['signal_total'] ?>">
              <?php foreach (Signal::SIGNALS as $k => $m): ?><button type="button" class="votebtn votebtn--mini" data-signal="<?= e($k) ?>" style="--c:<?= e($m['colour']) ?>" title="<?= e($m['label']) ?> · <?= e($m['blurb']) ?>" aria-label="<?= e($m['label']) ?>"><span aria-hidden="true"><?= e($m['icon']) ?></span><b data-count="<?= e($k) ?>"><?= (int)$s['signal_counts'][$k] ?></b></button><?php endforeach; ?>
            </div>
