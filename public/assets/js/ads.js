...
(function () {
  ...
  async function loadAdmin() {
    ...
    settings.innerHTML = `<span class="kicker">Currency &amp; payment gateways</span>
      ...
      <p class="panel__note">Stripe: <b class="${pricing.stripe ? 'is-good' : 'is-bad'}">${pricing.stripe ? 'connected' : 'add STRIPE_SECRET_KEY + STRIPE_WEBHOOK_SECRET to .env'}</b> · PayPal: <b class="${pricing.paypal ? 'is-good' : 'is-bad'}">${pricing.paypal ? 'connected (' + (pricing.paypal_mode || 'sandbox') + ')' : 'add PAYPAL_CLIENT_ID, PAYPAL_CLIENT_SECRET, PAYPAL_WEBHOOK_ID to .env'}</b>. Webhooks: <code>/api/stripe/webhook</code> and <code>/api/paypal/webhook</code>. Adverts are sold as impression packages; edit the packages below.</p>`;
