...
**Languages** — a globe switcher in the header (and the mobile drawer) offers English (UK), English (US), Irish, Polish, German, Ukrainian and Russian. Menus, buttons and the footer are translated natively from `config/lang/*.json` (add a key to any file to translate more of the chrome); for the non-English languages the article text is translated in the browser by Google Translate, with a notice bar and a one-tap way back to the English original. The choice is a cookie (`me_lang`) and works for signed-out readers.
...
**Email delivery** — **Settings → Email delivery** in the newsroom picks the transport and proves it works: a log file (nothing sent), PHP `mail()`, SMTP with STARTTLS/SSL, or **Brevo's transactional API** over HTTPS for hosts that block mail ports. Brevo also works as an ordinary relay (`smtp-relay.brevo.com:587`). The SMTP password and Brevo API key are encrypted with the same libsodium store as the payment keys and shown only as their last four characters; the panel has a "Send a test email" button, reports the last failure in plain words, and every message falls back to `storage/logs/mail.log` if a transport fails so nothing is lost. `.env` values (`MAIL_TRANSPORT`, `MAIL_FROM`, `SMTP_*`) still work as a fallback.

**Sharing** — a small row of share buttons on every story and notice (WhatsApp, Facebook, X, email, copy link) with a "More" sheet holding 30+ networks, a searchable list, the link to copy and Print. On phones the device's own share sheet leads, and app-only entries (Messenger, Viber, text message) appear only where they work. Everything is a plain link built from the page's own URL: no AddThis, no third-party script, no tracking, nothing extra to load. Add or reorder networks in `src/Support/Share.php`.

**Site address** — canonical URLs (sitemap, `<link rel=canonical>`, Open Graph, feeds, emails, payment return URLs) come from **Settings → Site address** in the newsroom. Leave it blank and the site follows whichever domain the visitor used; set it to pin one canonical domain. A `PUBLIC_BASE_URL` in `.env` is still honoured, but only when its host matches the live request, so a stale staging address can never poison the sitemap after a domain move.
